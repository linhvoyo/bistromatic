...
int							div_handle_negs(char **dividend,
		char **divisor, char **sign);
...
char						*get_closest_base(linked_list *list,
...
char						*replace(char *input,
							char *replaced, char *replacerint, int repeat);
...
char						*translate_to_decimal(char *charset,
							char *expression);
int							zeroed(char *str);
char						*value(linked_list *list, int n);
